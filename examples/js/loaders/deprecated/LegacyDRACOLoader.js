 ...
 */
THREE.LegacyDRACOLoader = function ( manager ) {
...
THREE.LegacyDRACOLoader.prototype = {
	constructor: THREE.LegacyDRACOLoader,
	...
	decodeDracoFile: function (
		...
		var scope = this;
		THREE.LegacyDRACOLoader.getDecoderModule().then( function ( module ) {
	...
	decodeDracoFileInternal: function (
		...
		if ( geometryType == dracoDecoder.TRIANGULAR_MESH ) {
			...
			var errorMsg = "THREE.LegacyDRACOLoader: Unknown geometry type.";
	...
	addAttributeToGeometry: function (
		...
		if ( attribute.ptr === 0 ) {
			...
			var errorMsg = "THREE.LegacyDRACOLoader: No attribute " + attributeName;
			console.error( errorMsg );
			throw new Error( errorMsg );
		...
		switch ( attributeType ) {
			...
			default:
				var errorMsg = "THREE.LegacyDRACOLoader: Unexpected attribute type.";
				console.error( errorMsg );
				throw new Error( errorMsg );
	...
	convertDracoGeometryTo3JS: function (
		...
		if ( ! decodingStatus.ok() || dracoGeometry.ptr == 0 ) {
			...
			var errorMsg = "THREE.LegacyDRACOLoader: Decoding failed: ";
		...
		if ( posAttId == - 1 ) {
			...
			var errorMsg = "THREE.LegacyDRACOLoader: No position attribute found.";
			console.error( errorMsg );
			dracoDecoder.destroy( decoder );
			dracoDecoder.destroy( dracoGeometry );
			throw new Error( errorMsg );
...
THREE.LegacyDRACOLoader.decoderPath = "./";
THREE.LegacyDRACOLoader.decoderConfig = {};
THREE.LegacyDRACOLoader.decoderModulePromise = null;

/**
 * Sets the base path for decoder source files.
 * @param {string} path
 */
THREE.LegacyDRACOLoader.setDecoderPath = function ( path ) {

	THREE.LegacyDRACOLoader.decoderPath = path;
 ...
 * @param {Object} config
 */
THREE.LegacyDRACOLoader.setDecoderConfig = function ( config ) {

	var wasmBinary = THREE.LegacyDRACOLoader.decoderConfig.wasmBinary;
	THREE.LegacyDRACOLoader.decoderConfig = config || {};
	THREE.LegacyDRACOLoader.releaseDecoderModule();

	// Reuse WASM binary.
	if ( wasmBinary ) THREE.LegacyDRACOLoader.decoderConfig.wasmBinary = wasmBinary;
 ...
 */
THREE.LegacyDRACOLoader.releaseDecoderModule = function () {
 ...
 */
THREE.LegacyDRACOLoader.getDecoderModule = function () {

	var scope = this;
	var path = THREE.LegacyDRACOLoader.decoderPath;
	var config = THREE.LegacyDRACOLoader.decoderConfig;
	var promise = THREE.LegacyDRACOLoader.decoderModulePromise;
	...
	if ( typeof DracoDecoderModule !== "undefined" ) {
		...
		// Load with asm.js.
		promise = THREE.LegacyDRACOLoader._loadScript( path + "draco_decoder.js" );

	} else {

		// Load with WebAssembly.
		config.wasmBinaryFile = path + "draco_decoder.wasm";
		promise = THREE.LegacyDRACOLoader._loadScript( path + "draco_wasm_wrapper.js" )
			.then( function () {

				return THREE.LegacyDRACOLoader._loadArrayBuffer( config.wasmBinaryFile );
	...
	THREE.LegacyDRACOLoader.decoderModulePromise = promise;
	return promise;
 ...
 * @return {Promise}
 */
THREE.LegacyDRACOLoader._loadScript = function ( src ) {
 ...
 * @return {Promise}
 */
THREE.LegacyDRACOLoader._loadArrayBuffer = function ( src ) {
