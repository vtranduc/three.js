/**
 * Generated from 'examples/jsm/loaders/IndexedFBXLoader.js'
 */

(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('three'), require('/Users/k/Workspace/spinvr/three.js/examples/jsm/libs/inflate.module.min.js'), require('/Users/k/Workspace/spinvr/three.js/examples/jsm/curves/NURBSCurve.js')) :
	typeof define === 'function' && define.amd ? define(['exports', 'three', '/Users/k/Workspace/spinvr/three.js/examples/jsm/libs/inflate.module.min.js', '/Users/k/Workspace/spinvr/three.js/examples/jsm/curves/NURBSCurve.js'], factory) :
		...
		GeometryParser.prototype = {
			...
			genBuffers: function ( geoInfo, vertexIndexMapping = {} ) {
				...
				var presentAttrs = [
					...
					return !! geoInfo[ attrName ];

				} );

				if ( geoInfo.skeleton ) {

					presentAttrs.push( "vertexWeights", "weightsIndices" );

				}

				if ( geoInfo.material && geoInfo.material.mappingType !== 'AllSame' ) {

					presentAttrs.push( "materialIndex" );

				}

				// Treat uvs separately, index them as uv_0, or uv_1, etc.
				if ( geoInfo.uv ) {

					geoInfo.uv.forEach( function ( _, uvIdx ) {

						var attrName = 'uv_' + uvIdx;
						buffers[ attrName ] = [];
						presentAttrs.push( attrName );

					} );

				}

				var visited = new Uint8Array( geoInfo.vertexPositions.length );
				...
				presentAttrs.forEach( function ( attrName ) {
				...
				geoInfo.vertexIndices.forEach( function ( inVertexIndex, polygonVertexIndex ) {
					...
					for ( let index = 0; index < 3; ++ index ) {
						...
						vertexData.vertex[ index ] = geoInfo.vertexPositions[ vertexIndex * 3 + index ];

					}

					if ( geoInfo.color ) {

						var color = getData( polygonVertexIndex, polygonIndex, vertexIndex, geoInfo.color );
						vertexData.colors = [ color[ 0 ], color[ 1 ], color[ 2 ] ];
					...
					if ( geoInfo.skeleton ) {
						...
						for ( var i = 0; i < 4; ++ i ) {
							...
							vertexData.vertexWeights[ i ] = weights[ i ];
							vertexData.weightsIndices[ i ] = weightIndices[ i ];
					...
					if ( geoInfo.normal ) {
						...
						var normal = getData( polygonVertexIndex, polygonIndex, vertexIndex, geoInfo.normal );
						for ( var i = 0; i < 3; ++ i ) {

							vertexData.normal[ i ] = normal[ i ];
					...
					if ( geoInfo.uv ) {
						...
						geoInfo.uv.forEach( function ( uv, i ) {
							...
							var uvData = getData( polygonVertexIndex, polygonIndex, vertexIndex, uv );
							vertexData[ 'uv_' + i ][ 0 ] = uvData[ 0 ];
							vertexData[ 'uv_' + i ][ 1 ] = uvData[ 1 ];
					...
					if ( needNewVertex ) {
						...
						// Copy our position data into the buffers
						vertexData.vertex.forEach( function ( position, i ) {

							buffers.vertex[ vertexIndex * 3 + i ] = position;
						...
						presentAttrs.forEach( function ( attrName ) {
							...
							for ( let dataIdx = 0; dataIdx < data.length; dataIdx ++ ) {
								...
								buffers[ attrName ][ vertexIndex * data.length + dataIdx ] = data[ dataIdx ];
				...
				if ( geoInfo.uv ) {
					...
					uvs = geoInfo.uv.map( function ( _, i ) {

						return buffers[ 'uv_' + i ];
			...
			genMorphGeometry: function ( parentGeo, newNumVertices, morphGeoNode, preTransform, name, vertexIndexMapping ) {
				...
				var positionAttribute = new THREE.Float32BufferAttribute( morphPositions, 3 );
